  ...
  return (
    <header className="app-header">
      ...
      <nav className="toolbar" aria-label="Project actions">
        ...
        <button className="button button--ghost" disabled={!hasAutosave} type="button" onClick={onRestoreAutosave}>
          <FolderOpen size={16} />
          Restore
        </button>
        <div className="export-menu" aria-label="Export subtitles">
          <button className="button button--soft" disabled={!entryCount} type="button" onClick={() => onExport('srt')}>
            <FileDown size={16} />
            SRT
          </button>
          <button className="button button--soft" disabled={!entryCount} type="button" onClick={() => onExport('vtt')}>
            <FileText size={16} />
            VTT
          </button>
          <button className="button button--soft" disabled={!entryCount} type="button" onClick={() => onExport('txt')}>
            TXT
          </button>
          <button className="button button--soft" disabled={!entryCount} type="button" onClick={() => onExport('json')}>
            <FileJson size={16} />
            JSON
          </button>
        </div>
        <IconButton label="Clear autosave" disabled={!hasAutosave} onClick={onClearAutosave}>
          <Trash2 size={16} />
        </IconButton>
